...
# CONFIGURATION
# ==============================================================================
DATASET_FILE = "motion_dataset_loso_v6.npz"
MODEL_SAVE_NAME = "best_motion_model_v6.keras"

# Architecture
CONV_FILTERS_BLOCK1 = 64
CONV_FILTERS_BLOCK2 = 128
CONV_FILTERS_BLOCK3 = 128       # Increased from 64: More high-level capacity
KERNEL_SIZE_BLOCK1 = 5           # Wider kernel: captures more context per step in shorter window
KERNEL_SIZE_BLOCK2 = 3
KERNEL_SIZE_BLOCK3 = 3
POOL_SIZE = 2
DROPOUT_CONV = 0.3               # Lower dropout in conv: augmented data acts as regularizer
DROPOUT_DENSE = 0.4              # Higher in dense: prevents overfit on classification head
DENSE_UNITS_1 = 128              # Extra dense layer for more classification capacity
DENSE_UNITS_2 = 64

# Training hyperparameters
MAX_EPOCHS = 100                 # More epochs: augmented data prevents overfitting
BATCH_SIZE = 64
LEARNING_RATE = 0.001            # Higher initial LR: cosine annealing will handle decay
LABEL_SMOOTHING = 0.1            # Reduces overconfidence, improves generalization
EARLY_STOPPING_PATIENCE = 25     # Generous patience with cosine annealing
L2_REGULARIZATION = 0.0005       # Lighter L2: augmentation handles regularization

MODEL_REPORT_FILE = "model_v6_report.txt"
...
# ==============================================================================
# 2. BUILD THE V6 CNN MODEL
# ==============================================================================
# Improvements over V5:
# - Wider first kernel (5) to capture more temporal context in shorter window
# - Extra dense layer (128 → 64) for more classification capacity
# - Tuned dropout (lower in conv, higher in dense)
...
model = models.Sequential([
    ...
    # important for shorter windows that need to extract maximum info.
    ...
    # Standard kernel (3) combines block 1 features into step patterns.
    ...
    # 128 filters (up from 64 in v5) to capture richer high-level patterns
    # for distinguishing similar activities (walking vs upstairs).
    ...
    # Extra dense layer gives the model more capacity to learn
    # non-linear decision boundaries between similar classes.
    ...
    layers.Dense(DENSE_UNITS_2, activation='relu',
                 kernel_regularizer=tf.keras.regularizers.l2(L2_REGULARIZATION)),
    layers.Dropout(DROPOUT_CONV),  # Lighter dropout before final layer
...
metadata = {
    ...
    "dataset_info": {
        ...
        "augmentation": "jitter + scaling + time_shift (2 copies)"
    }
}

with open(MODEL_REPORT_FILE, 'w') as f:
    f.write("=" * 70 + "\n")
    f.write("MODEL V6 - TRAINING REPORT\n")
    f.write("=" * 70 + "\n\n")
